...
def metrics():
    ...
    for node in NODES:
        ...
        #Esto es un pequeño arreglo visual para que salgan iconos en los nodos, si os gusta mas sin iconos lo podemos sacar

        if node == "nginx":
                
            out.append(
            f'nodegraph_node_info{{id="{node}",title="{node}",subtitle="{subtitle}",'
            f'mainstat="{status}",icon="cloud",secondarystat="{lat_str}",color="{color}"}} 1'
            )           

        else: 
            out.append(
            f'nodegraph_node_info{{id="{node}",title="{node}",subtitle="{subtitle}",'
            f'mainstat="{status}",icon="monitor",secondarystat="{lat_str}",color="{color}"}} 1'
            )

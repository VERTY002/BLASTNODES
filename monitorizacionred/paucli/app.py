...
latencia_mensajes = Histogram('latencia_mensajes_segundos', 'Tiempo de ida y vuelta de mensajes', ['destino'])
        ...
        for peer in PEERS:
            ...
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    ...
                    latencia = end - start
                    latencia_mensajes.labels(destino=peer).observe(latencia)
                    print(f"[{NODE_NAME}] Respuesta de {peer}: {response}, Latencia: {latencia:.3f}s")
